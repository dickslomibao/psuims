@include('admin.includes.header', ['title' => $matrix_citeria->title])
    ...
    <div class="row">
        <div class="col-12">
            <div class="w-100 d-flex justify-content-between table-header-btn">
                <div></div>
                @if ($percentage < $matrix_citeria->percentage)
                    <button class="btn-add-management" id="modal_btn" data-bs-toggle="modal"
                        data-bs-target="#addCampusForm"><i class="fa-solid fa-plus"></i> Add matrix sub criteria</button>
                @endif
            </div>
    ...
    <div class="modal-dialog">
        <div class="modal-content">
            <div class="modal-header">
                <h1 class="modal-title fs-5" id="addCampusFormLabel">Add new sub criteria</h1>
            ...
            <div class="modal-body">
                <form method="POST" id="add-criteria-form">
                    ...
                    <div class="mb-3">
                        ...
                        <input type="number" name="percentage" required class="form-control" id="percentage"
                            aria-describedby="emailHelp">
                    </div>
...
<script>
    $("#add-criteria-form").validate({
        rules: {
            ...
            percentage: {
                required: true,
                max: parseFloat('{{ $matrix_citeria->percentage - $percentage }}'),
            },
        },
        messages: {
            title: {
        ...
        submitHandler: function(form) {
            ...
            $.ajax({
                type: "POST",
                url: `{{ route('add.matrix_sub_criteria', [
                    'matrix_id' => $matrix->id,
                    'matrix_criteria_id' => $matrix_citeria->id,
                ]) }}`,
                contentType: false,
                processData: false,
                data: formData,
    ...
    <ul class="dropdown-menu">
        <li><a class="dropdown-item" >View Criteria </a></li>
    </ul>
</div>`;
                        },
